...
def clean_text(text):
    ...
    text = text.lower()
    # Remove special characters but keep numbers and essential medical units
...
def main():
    # 1. Load the data
    # Assuming headers are provided as described
    print("Loading data...")
    try:
        df_drugs = pd.read_csv('/home/qlunlp/ylh/database/eICU/process/tmp/unique_drugname.csv', header=None, names=['drugname'])
        # For rxnorm, assuming tab-separated or pipe-separated based on typical medical files. 
        # Adjust 'sep' if it's comma-separated.
        df_rxnorm = pd.read_csv('/home/qlunlp/ylh/database/eICU/process/tmp/processed_rxnorm_mappings.csv', sep=',', names=['RXCUI', 'RXSTRING'], dtype={'RXCUI': str})
    ...
    df_drugs['clean_name'] = df_drugs['drugname'].apply(clean_text)
    df_rxnorm['clean_name'] = df_rxnorm['RXSTRING'].apply(clean_text)

    # Drop duplicates in reference data to speed up matching (we only need unique strings)
    df_ref = df_rxnorm.drop_duplicates(subset=['clean_name']).reset_index(drop=True)
    
    # 3. Vectorization (TF-IDF)
    # Character n-grams (3-gram) are often better for drug names to catch partial misspellings 
    # or small variations (e.g., "sulfate" vs "sulphate")
    print("Vectorizing text...")
    vectorizer = TfidfVectorizer(analyzer='char', ngram_range=(3, 3))
    
    # Fit on reference data (the larger dataset)
    tfidf_ref = vectorizer.fit_transform(df_ref['clean_name'])
    tfidf_query = vectorizer.transform(df_drugs['clean_name'])

    # 4. Nearest Neighbor Search
    # We use NearestNeighbors with cosine distance
    print("Finding best matches...")
    nbrs = NearestNeighbors(n_neighbors=1, metric='cosine', n_jobs=-1)
    nbrs.fit(tfidf_ref)
    
    distances, indices = nbrs.kneighbors(tfidf_query)
    ...
    for i, match_index in enumerate(indices):
        query_drug = df_drugs.iloc[i]['drugname']
        best_match_idx = match_index[0]
        
        # Get the matched string and original RXCUI
        matched_clean_str = df_ref.iloc[best_match_idx]['clean_name']
        
        # Find the RXCUI corresponding to this matched string in the original rxnorm dataframe
        # (There might be multiple RXCUIs for the same string, we usually take the first one found)
        rxcui = df_rxnorm[df_rxnorm['clean_name'] == matched_clean_str]['RXCUI'].iloc[0]
        
        result_dict[query_drug] = rxcui
        
        # Optional: Print a few examples to verify accuracy
        if i < 5:
            print(f"Match: {query_drug} -> {rxcui} (Matched Text: {df_ref.iloc[best_match_idx]['RXSTRING']})")

    # Output result
    print(f"Processed {len(result_dict)} drugs.")
    return result_dict

if __name__ == "__main__":
    drug_mapping = main()
    # You can save this dict to json or use it directly
    # 字典转化为csv
    pd.DataFrame(list(drug_mapping.items()), columns=['drugname', 'RXCUI']).to_csv('/home/qlunlp/ylh/database/eICU/process/tmp/drug_rxcui_mapping.csv', index=False)